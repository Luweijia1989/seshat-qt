...
#include "meparser.h"
void ProductionB::printOut(meParser *parser, Grammar *G, Hypothesis *H) {
    if( outStr ) {
...
//                putchar(outStr[i]);
                parser->result += outStr[i];
                i++;
            }
            i+=2;

            if( H->hd->clase < 0 )
                H->hd->prod->printOut(parser, G, H->hd );
            else {
//                printf("%s", H->hd->pt->getTeX( H->hd->clase ) );
                parser->result.append(H->hd->pt->getTeX( H->hd->clase ));
            }
...
//                putchar(outStr[i]);
            ...
            if( H->hi->clase < 0 )
                H->hi->prod->printOut(parser, G, H->hi );
            else {
//                printf("%s", H->hi->pt->getTeX( H->hi->clase ) );
                parser->result.append(H->hi->pt->getTeX( H->hi->clase ));
            }
        }
        else {
            if( pd1 >= 0 ) {
